...
export default function TodoList() {
    ...
    const [selectListById, setSelectListById] = React.useState<Set<string>>(
        new Set(),
    );

    const { data } = useQueryTodoList();

    const TODO_LIST_CONTENTS = [
        'bg-[#fafafa]',
    ...
    const handleTodoListClick = (id: string) => {
        const updatedSelectListById = new Set(selectListById);
        if (updatedSelectListById.has(id)) {
            updatedSelectListById.delete(id);
        } else {
            updatedSelectListById.add(id);
        }
        setSelectListById(updatedSelectListById);
    };

    console.log(selectListById, 'selectListById ?');
        ...
        <section className="flex flex-col items-center w-full h-full py-[1rem]">
            ...
            <section className="flex w-full h-[10rem] gap-[0.3rem] pt-[0.5rem]">
                <div className="flex-1 flex flex-col h-full gap-2">
                    ...
                    />
                    <textarea
                        data-cy="todolist_textarea_content"
                    ...
                    {formatData &&
                        formatData.map((list: TodoInterface) => (
                            ...
                            >
                                    ...
                                    <div className="flex gap-[0.6rem] pr-[0.5rem]">
                                        {isEditById(list.id) ? (
                                            ...
                                            <>
                                                ...
                                                <button
                                                    data-cy="todolist_delete_button"
                                                    type="button"
                                                    onClick={() =>
                                ...
                                <div
                                    className={
                                        selectListById.has(list.id)
                                            ? cx(TODO_LIST_CONTENTS, 'h-[50px]')
                                            : cx(TODO_LIST_CONTENTS, 'h-[0px]')
                                    }
                                >
                                    <span className="text-[14px] text-[gray] px-8 py-2">
                                        {list.content}
                                    </span>
                                </div>
                            </article>
